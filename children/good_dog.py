...
def process_message(msg: holly.ParsedHollyMessage):
    ...
    if msg.match([]) and msg.is_targeted():
        responses = ['yes?', 'wut', 'That\'s me!', 'I\'m Holly!',
                     '*jumps up to lick your face*', 'hmmmm?', 'bork', 'Do you have a treat?']
        return random.choice(responses)

    if msg.match("thanks holly") or msg.match("thank you holly"):
        responses = ['yw', 'ofc', 'Yes, now give me a treat', 'Ok, can we go on a walk now?', '*jumps up to lick your face*',
                     'Anything for my favorite humans', 'bork', 'Just doing my job. Now give me a treat *begging eyes*']
        return random.choice(responses)

    if msg.is_targeted() and msg.match("shake"):
        responses = ['Only if you have a treat', '*raises paw*', '*raises paw, but menacingly*',
                     'lol', '*shakes aggressively*', '*happily put paw in your hand*']
        return random.choice(responses)

    if msg.is_targeted() and msg.match("roll over"):
        responses = ['Only if you have a treat', 'weeeeeeeeee',
                     '*bark bark*', 'lol', '*rolls over*', 'I\'m too dizzy']
        return random.choice(responses)

    if msg.is_targeted() and msg.match("go to bed"):
        responses = ['Only if you have a treat', 'One step ahead of you',
                     'zzzzzzzzzzzzzz', 'Don\'t need to ask me twice', 'k bet']
        return random.choice(responses)
    ...
    if msg.match("squirrel"):
        responses = ['*BARK BARK BARK BARK BARK BARK BARK*', '*bolts away*',
                     'Ok, and?', 'Nice try, I\'m not running that far.', ascii_dogs.RUNNING]
        return random.choice(responses)

    if msg.match("cat"):
        responses = ['*BARK BARK BARK BARK BARK BARK BARK*', '*bolts away*', 'Ok, and?',
                     'Nice try, I\'m not running that far.', 'My mortal enemy...', 'I must defend the humans!', ascii_dogs.RUNNING]
        return random.choice(responses)
